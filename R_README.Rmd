...
# Making the plot, adding a little pink in the plot, cause I like pink, and rainbow, cause its fun :)
...
Density of Positions on Each Chromosome
```{r}
...
ggplot(merged_genotypes) + geom_density(mapping = aes(x=Position, fill = Chromosome, alpha = 0.05)) + ggtitle("Density of Position by Chromosome") + theme(
...
# This graph shows you the density of positions on each chromosome by chromosomes. Kinda messy, but it visualizes where the SNPs are on each chromosome and helps compare chromosomes. It is much cleaner than the column plot that I tried :).
```
